...
var urlSlugs = []; //name of the new list of slugs
for (i = 0; i < crayola1990.length; i++) { //comb through crayola1990 array
    var crayola1990lc = crayola1990[i].toLowerCase(); //makes it lowercase
    var updated = "";
    for (j = 0; j < crayola1990lc.length; j++) { //cycles through new lowercase array
        var dash=""
        if (crayola1990lc[j] === " ") {
            dash += "-";
        } else {
            dash += crayola1990lc[j];
        };
        updated += dash;
    };
    urlSlugs.push(updated);
};
